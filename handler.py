...
browsers = get_configuration_api_names('Win7x64-C1')
...
for i in range(len(si_list)):
    ...
    for j in range(len(browsers)):
        try:
            ...
            if not tested:
                passed = tetest.run_test('220680', 'chase@crossbrowsertesting.com', 'u81305495037f796', si_list[i][0],'Win7x64-C1', browsers[j][1], '540aaff6e6dea3e442968d24cce4a819', 0)
                # print(passed)
                if passed:
